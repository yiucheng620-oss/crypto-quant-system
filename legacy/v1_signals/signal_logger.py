...
# Indicator configs — 14 total: 7 Long + 7 Short, WFA-validated + experimental
# V2 2026-05-18: Expanded from 7→14 indicators to increase signal density
# Target: ≥30 settled signals/90d (was ~10)
INDICATORS = {
    # ══════ LONG (7) ══════
    ...
    # VP_BTC_1h_L — experimental, VP logic on 1h for faster signals
    "VP_BTC_1h_L": {
        "coin": "BTC", "tf": "1h", "fn": "vp_signal", "direction": "LONG",
        "params": {"vp_lookback": 15, "dist_pct": 0.8, "vol_thresh": 1.5, "vol_lookback": 20},
    },
    "OBV_BTC_1h_L": {
    ...
    # OBV fast variants — cooldown 4h for more signals
    "OBV_BTC_1h_L_fast": {
        "coin": "BTC", "tf": "1h", "fn": "obv_signal", "direction": "LONG",
        "params": {"smooth": 6, "diverge_lookback": 8, "sig_mult": 1.1, "cooldown": 4},
    },
    "OBV_SOL_1h_L_fast": {
        "coin": "SOL", "tf": "1h", "fn": "obv_signal", "direction": "LONG",
        "params": {"smooth": 6, "diverge_lookback": 8, "sig_mult": 1.1, "cooldown": 4},
    ...
    # OBV short — divergence-based short signals
    "OBV_BTC_1h_S": {
        "coin": "BTC", "tf": "1h", "fn": "obv_short_signal", "direction": "SHORT",
        "params": {"smooth": 8, "diverge_lookback": 10, "sig_mult": 1.3, "cooldown": 8},
    },
    "OBV_SOL_1h_S": {
        "coin": "SOL", "tf": "1h", "fn": "obv_short_signal", "direction": "SHORT",
        "params": {"smooth": 8, "diverge_lookback": 10, "sig_mult": 1.3, "cooldown": 8},
    },
    # VP_ETH_1h_S — 1h VP short for ETH
    "VP_ETH_1h_S": {
        "coin": "ETH", "tf": "1h", "fn": "vp_short_signal", "direction": "SHORT",
        "params": {"vp_lookback": 15, "dist_pct": 0.5, "vol_thresh": 1.5, "vol_lookback": 20},
    },
}
...
def generate_accuracy_csv(days: int = 90):
    ...
    # Per-indicator accuracy — NO minimum threshold, include all data
    # (was HAVING total >= 3 before 2026-05-18, which filtered out 5/6 indicators)
    c.execute("""
...
def _build_summary_json(days: int = 90) -> dict:
    ...
    # Per-market-condition breakdown (matrix) — NO minimum threshold
    # (was HAVING total >= 3 before 2026-05-18)
    c.execute("""
    ...
    # Current market conditions (all 3 coins — was missing BTC before 2026-05-18)
    current_state = {}
    ...
    # Pending signals (not yet checked — added 2026-05-18)
    c.execute("""
        SELECT s.indicator, s.coin, s.timeframe, s.direction, s.price,
               s.timestamp, COUNT(*) as pending_count
        FROM signals s
        LEFT JOIN accuracy a ON s.id = a.signal_id
        WHERE a.id IS NULL
        GROUP BY s.indicator, s.coin, s.timeframe, s.direction
...
def main():
    ...
    for ind_name, config in INDICATORS.items():
        ...
        # Check regime fitness before generating signals
        regime_blocked = False
        try:
            from regime_selector import get_selector
            sel = get_selector()
            smap = sel.get_selection_map()
            # Find matching regime key
            for key, inds in smap.items():
                if key.startswith(f"{coin}_"):
                    entry = inds.get(ind_name, {})
                    if not entry.get("selected", False) and entry.get("confidence_rank", 0) == 0:
                        regime_blocked = True
                        reason = entry.get("warning", "regime mismatch")
                        print(f"  [BLOCKED] {ind_name}: regime={key.split('_',1)[1]}, {reason}")
                    break
        except ImportError:
            pass  # Regime selector not available, proceed normally
